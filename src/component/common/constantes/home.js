export const HEAD = ["","adeline.simon31@hotmail.fr", "0621692869","adeline-simon-b8614018b","isyviel","Version papier"]
export const ICONS = ["images/mail.png","images/phone.png","images/linkedin.png","images/github.png","images/pdf.png"]
export const QUOTE = ["Faire un bond en avant, je sautille pas","- Perceval, Kaamelott"]
export const NOM = "Adeline SIMON"
export const TITRE = "Développeuse Web et Web Mobile"
export const TECHNOS = ["ReactJS, ","PHP 7, ","Symfony 4, ","MySQL, ","Git, "," Material-ui, ","Boostrap"]
export const MENU ={
    exp: {
        index: 0,
        title:"EXPERIENCE",
        classe: "justify-content-center align-items-center",
    },
    formations: {
        index: 1,
        title: "FORMATIONS",
        classe: "justify-content-md-start justify-content-center align-items-center",
    },
    projet : {
        index: 2,
        title: "PORTFOLIO",
        classe: "justify-content-center align-items-center",
    },
    mail : {
        index: 3,
        title: "CONTACT",
        classe: "justify-content-md-start justify-content-center align-items-center",
    }
}

export const STRATEGIA = {
    title: "STRATEGIA",
    href: "https://strategia.io/",
    content: "Site Web STRATEGIA",
    skills: "Symfony, React/JSX, ApiPlatform, Material-UI, Postman, Twig, Bootstrap, Ubuntu",
    softSkills: "Autonomie, communication, passion, implication",
    src : "images/strategia.png",
    alt : "logo strategia",
}

export const OCCITECH = {
    title: "OCCITECH",
    href: "https://www.occitech.fr/",
    content: "Site Web OCCITECH",
    skills: "Magento 1 & 2, Symfony, CakePHP, React/JSX, Front Commerce, Tailwind, , ArchLinux"
    softSkills: "Autonomie, communication, passion, implication",
    src : "images/occitech.png",
    alt : "logo occitech",
}

export const VENTE = {
    title: "Mim / Christine Laure",
    content: "2009-2019",
    skills: "Commerce, Manutention, Comptabilité",
    softSkills: "Autonomie, transparence, dynamisme, proactivité",
    src : "images/MIM.jpg",
    alt : "logo mim et christine laure",
}
export const ADRAR = {
    title: "Développeuse Web et Web Mobile",
    href: "https://www.adrar-numerique.com/",
    content: "Site Web Adrar",
    skills: "Titre Professionnel niveau III",
    softSkills: "Adrar Pôle Numérique - Ramonville St Agne",
    src : "images/adrar.png",
    alt : "logo adrar",
}

export const ANGLAIS = {
    title: "Anglais",
    content: "2011",
    skills: "Bac+2 Langue Littérature et Civilisation Etrangère Anglaise",
    softSkills: "Université Lettres et Langues Poitiers",
    src : "images/univ.png",
    alt :"logo université poitiers",
}

export const STRList = {
    skills: "Méthode agile, Slack, Miro, Invision, GitLab.", 
    resume: "Conception et développement de fonctionnalités sur une plateforme de gestion de workflow:",
    detales : [ "Insertions de champs de saisie selon le type de demande.","Suppression, duplication, exportation en pdf d'une demande.",
        "Adaptation de la saisie, des données accessibles et des actions possibles selon l'utilisateur.",
        "Processus de validation du workflow",
        "Reproductions d'erreurs et tests.",
        "Responsive"],
}

export const OCCList = {
    skills: "Méthode agile, Slack, Miro, Invision, GitLab.", 
    resume: "Conception et développement de fonctionnalités sur une plateforme de gestion de workflow:",
    detales : [ "Insertions de champs de saisie selon le type de demande.","Suppression, duplication, exportation en pdf d'une demande.",
        "Adaptation de la saisie, des données accessibles et des actions possibles selon l'utilisateur.",
        "Processus de validation du workflow",
        "Reproductions d'erreurs et tests.",
        "Responsive"],
}

export const AList = {
    conception : {
        title : "Conception et maquettage d'applications:",
        detales : ["Diagrammes de classes, de séquences, d'activités","Dictionnaires de données", "MCD, MLD, SQL"],
    },
    back : {
        title: "Gestion la partie back end d'un site web ou d'une application mobile:",
        detales: "Java,PHP",
    },
    front: {
        title: "Construction et intégration d'une interface web dynamique:",
        detales: "Android, Javascript (Jquery, React), HTML5, CSS3, Bootstrap",
    }
}

export const VList = {
    skills: "Accueil personnalisé, conseils, accompagnement et vente complémentaire.",
    detales: [
        "Mise en place du merchandising libre ou imposé.",
        "Gestion des livraisons, des retours dépôts et des inventaires.",
        "Calcul des objectifs vendeuses et magasin.",
        "Gestion de la sécurité du personnel et des fonds.",
        "Organisation de mesures contre la démarque inconnue."
    ]
}

export const ANGList = {
    detales:  ["Littérature",
    "Phonétique",
    "Linguistique",
    "Civilisation",
    "Compréhension et Expression orale",
    "Options : Science du langage et Cinéma"]
}

